 ...
 */
export async function fetchCurrencies(base: string = 'USD'): Promise<NewPriceParams[]> {
  ...
  return Object.keys(rates).map((currency) => {
    return {
      ...
      base: currency,
      value: new Decimal(new BigNumber(1).dividedBy(new BigNumber(rates[currency])).toNumber()),
